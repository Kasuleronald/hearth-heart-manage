  ...
  return (
    <div>
      <Button asChild variant="ghost" size="sm" className="mb-2">
        <Link to="/members"><ArrowLeft className="mr-1 h-4 w-4" /> All members</Link>
      </Button>
      <PageHeader
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <Card><CardContent className="space-y-3 p-6">
          ...
          <Row label="Date of birth" value={member.dob ? format(new Date(member.dob), "PPP") : undefined} />
          <Row label="Joined" value={member.joinDate ? format(new Date(member.joinDate), "PPP") : undefined} />
        </CardContent></Card>
        <Card><CardContent className="space-y-3 p-6">
          ...
          <Row
            ...
            value={cell ? (
              <Link to="/cells/$id" params={{ id: cell.id }} className="text-primary hover:underline">
                {cell.name}
              </Link>
            ) : undefined}
          />
